...
I indexed transcripts with a privacy-safe session indexer that records **metadata only**: tool-call counts, token aggregates, compaction counts, model and role labels. It does **not** store prompts, tool outputs, or user content. Any session could be reconstructed and checked without exposing its contents.
...
2. Count tool calls by type. `exec`/`execute` should dominate.
