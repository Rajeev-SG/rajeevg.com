...
That should not be compared directly with the broader hackathon action mix visible on the GA4 dashboard:

- `1,337` consented tracked actions
- `313` non-consented tracked actions
- `96` tracked actions with no populated consent flag

That dashboard split uses a different denominator:

- consented tracked action share: `1,337 / (1,337 + 313) = 81.0%`
- non-consented tracked action share: `313 / (1,337 + 313) = 19.0%`
...
- the `page_context` split was mostly denied in one narrow page-level stream
- the broader known-consent hackathon action mix was mostly consented
- neither split explains vote-capture coverage on its own

That is why event-level consent metrics are useful for understanding measurement conditions, but dangerous if someone mistakes them for a clean user count or a vote-capture rate.
...
- the clearest `page_context` consent split was mostly denied, but that was not the same as overall vote-capture coverage
