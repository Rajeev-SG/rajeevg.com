---
title: "Server-Side Tagging: When It's Worth It and When It Isn't"
slug: when-not-to-use-server-side-tagging
date: 2026-04-01
updated: 2026-09-05
description: "Should you run server-side tagging? A real Cloud Run cost autopsy from rajeevg.com plus a plain decision framework: when same-origin sGTM pays for itself, and when client-side GTM is the better call."
...
Most small content sites should **stay client-side only**. Server-side tagging starts making sense when at least one of these is true:

- paid media is a real, ongoing channel and the warm runtime cost would be trivial next to it,
- Safari and iOS together make up a large share of your audience (roughly above 20% combined), or
- you need server-only control: secret-backed routing, event filtering, PII scrubbing, or fan-out to multiple vendors.

Below is the full decision framework and the real billing data behind it. The cost autopsy comes from my own Cloud Run deployment on this site — what it actually cost, why it was mostly fixed charges, and why I shut it down.
...
## Why I would not use server-side tagging here (rajeevg.com case study)
...
- [Agentic data collection and warehousing](/blog/agentic-data-collection-and-warehousing) — where my measurement stack went after the teardown
- [How we built a consented first-party analytics stack](/blog/how-we-built-a-consented-first-party-analytics-stack) — the client-side GTM + consent setup I chose instead
- [How we finished the GA4 property setup](/blog/how-we-finished-the-ga4-property-setup-on-rajeevg-com) — the property-side configuration that makes client-side data trustworthy
