...
<Figure
  ...
  alt="Three-band draw.io loop showing the Agent Ops operating sequence: the daily band runs worktree, contract, session and scoped tools into acceptance and merge; the drift band routes failure through one alert channel, one runbook and a cause fix; the learning band feeds telemetry back into budget and routing rules, and the keeper restarts what died"
  caption="The full loop. The daily band is what you do every time work runs: worktree, contract, session, scoped tools, then acceptance proof and merge-or-discard. The drift band is what happens when anything shows up wrong: one alert channel, one runbook, and a fix aimed at the cause rather than this instance. The learning band is what most people skip: the fix, the contract miss, and the routing decision all feed back into the rules that govern the next run. Solid lines are the work path; dashed lines are what telemetry records; orange and red are the incident path; purple is the learning path."
  slug="agent-ops-operating-loop"
  editableHref="/downloads/agent-ops-operating-loop.drawio"
