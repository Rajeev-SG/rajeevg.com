...
date: 2026-03-23
updated: 2026-03-24
description: A historical walkthrough of the March 2026 server-side analytics build on rajeevg.com, before the live site was simplified back to client-side GTM and GA4 only.
...
> **Canonical update (2026-08-29)**: This article is a field note. The current consolidated story is [Agentic data collection and warehousing](/blog/agentic-data-collection-and-warehousing).
...
<div className="not-prose my-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
  ...
  <div className="rounded-2xl border bg-card p-4 shadow-sm">
    ...
    <p className="mt-2 text-lg font-semibold">`personal-gws-1:analytics_498363924`</p>
...
flowchart TB
  ...
  GA4 --> BQ["BigQuery export<br/>analytics_498363924"]
...
<ArticleFigure
  src="/images/blog/analytics-stack/gtm-web-workspace-tags.png"
  alt="Web GTM workspace showing the live Google tag base tag"
  eyebrow="Web GTM"
  title="A deliberately lean web GTM workspace"
  caption="The current live web container is intentionally sparse. This screenshot shows the active `Google tag - GA4 base` tag firing on `Initialization - All Pages`, which is the cleanest possible evidence for the browser-side bootstrap layer."
/>

### Live server-side GTM

<ArticleFigure
  src="/images/blog/analytics-stack/gtm-server-workspace-tags.png"
  alt="Server-side GTM versions page showing Version 3 live"
  eyebrow="Server GTM"
  title="Published server-side GTM versions are a better proof surface than the empty workspace"
  caption="For the server container, the versions page turned out to be the honest screenshot worth keeping. It shows `Version 3 is Live`, with the dependency-serving client setup and the earlier GA4 relay setup both visible in version history."
/>
...
- the first-party `/metrics` requests captured from the browser
...
<ArticleFigure
  ...
  caption="This refreshed screenshot comes from the live property after the definitions table fully loaded. The visible rows include `Analytics Consent State`, `Content Group`, `Content ID`, `Content Tags`, and `Page Type`, which is the kind of reporting surface the raw event contract needed."
/>
...
<ArticleFigure
  src="/images/blog/analytics-stack/looker-dashboard-audience-devices.png"
  alt="Looker Studio audience and devices page with country, device, operating system, and browser charts"
  eyebrow="Looker Studio"
  title="A live audience-and-devices page from the Looker reporting layer"
  caption="This replacement screenshot is narrower but far more honest: it shows a populated Looker Studio page with visible device, country, operating-system, and browser breakdowns instead of a sparse or half-empty canvas."
  imgClassName="max-h-[34rem]"
/>
