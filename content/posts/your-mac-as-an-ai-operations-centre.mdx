...
<figure className="not-prose my-10">
  <div className="relative left-1/2 w-[min(96vw,86rem)] -translate-x-1/2 overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700">
    <img
  ...
  <figcaption className="mx-auto mt-3 max-w-3xl text-center text-sm leading-6 text-slate-600 dark:text-slate-400">
    <span className="block sm:hidden">Swipe sideways to inspect the full boundary. </span>
...
<figure className="not-prose my-10">
  <div className="relative left-1/2 w-[min(96vw,86rem)] -translate-x-1/2 overflow-x-auto rounded-2xl border border-slate-200 bg-[#fbfaf5] shadow-sm dark:border-slate-700">
    <img
      src="/images/editorial/mac-ops-alarm-journey.svg"
      alt="Six-stage hand-drawn journey of an alarm from firing to confirmed recovery, with each stage annotated with the named stall point that breaks it and the named bound that holds it together"
      className="block min-w-[720px] w-full max-w-none sm:min-w-0"
    />
  </div>
  <figcaption className="mx-auto mt-3 max-w-3xl text-center text-sm leading-6 text-slate-600 dark:text-slate-400">
    <span className="block sm:hidden">Swipe sideways to read every stage. </span>
    The alarm journey, and where it actually stalls. Every stage can break: alert fatigue, a sleeping Mac, a lost tailnet, a missing runbook, an agent that lost scope, or counters that look green while the cause persists. Each stall has a named bound — one human channel, launchd's restart, the loopback perimeter, a runbook written during the incident, the worktree and contract, and telemetry checked rather than vibes.{" "}
    <a className="font-medium underline underline-offset-4" href="/downloads/mac-ops-alarm-journey.excalidraw">Download the editable Excalidraw source</a>. <span className="text-xs">Evidence date: 31 August 2026.</span>
  </figcaption>
</figure>

What the figure adds that the numbered list cannot: the failure layer. The list narrates the happy path; the figure shows, at every stage, where this journey actually goes wrong and what holds it together when it does. That is the honest version of the journey, and it is the version worth knowing before you rely on it.
