...
<div className="not-prose my-8 grid gap-4 sm:grid-cols-2">
  ...
  <div className="border-t-4 border-emerald-500 bg-slate-50 p-5 dark:bg-slate-900">
    ...
    <div className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">Runs Codex in headless mode, edits files, runs tests, pushes commits.</div>
  </div>
...
The Hetzner instance is a CX32: 4 vCPUs, 8 GB RAM, 80 GB disk, located in Germany. This is not high-end hardware. It is enough for two agents because each is relatively light when idle and scales up only when working.
...
- **HTTPS Coolify access**: The Coolify dashboard is reachable only via HTTPS, not plain HTTP
...
OpenClaw is a Telegram bot built on the Grammy framework. It receives messages, decides whether it needs search, calls the Brave Search API, and returns formatted responses.
...
Hermes runs OpenCode in headless mode inside a container. This means:
...
- **Gmail OAuth**: The integration for sending email summaries is built but awaits OAuth approval from Google. Until then, email alerts do not work.
- **Cloudflare API token**: A scoped token for DNS management exists but is pending authorization in the Cloudflare dashboard. Until then, DNS changes require manual updates.

These are not hypothetical future features. They are implemented but blocked on external approvals. I cannot control Google's OAuth review timeline or Cloudflare's token authorization flow.
...
The mailbox stores messages in a local database. I back this up daily to object storage. The agent code lives in Git. The environment variables are documented but not committed.

If the Hetzner server died tomorrow, I could rebuild in a few hours from Git and backups, not days from memory.
...
I configured UFW, but I also ran `ufw status` to verify. I set up HTTPS, but I also tried accessing over HTTP to confirm it failed. Configuration files lie. Verification catches the lies.
...
### 5. External approvals are unpredictable

The Gmail OAuth and Cloudflare token are not blocked by my code or my configuration. They are blocked by external processes. I cannot speed them up. The lesson is to build without them first, then add them when ready, rather than waiting to launch.

## Cost and performance

The Hetzner CX32 costs approximately €5.35 per month. Coolify is free and open source. The Brave Search API has a free tier that covers my usage. Telegram bot API is free. Vercel hobby tier covers the deployment.

Total running cost: under €6 per month, plus my time for maintenance.

Performance is acceptable for personal use. Research queries take 10-30 seconds depending on search depth. Engineering tasks vary by complexity. Neither agent is fast, but both are asynchronous: I send a request, do other work, and receive a notification when ready.
...
<div className="not-prose my-8 grid gap-5 md:grid-cols-2">
  ...
  <div className="border-t-4 border-amber-500 bg-slate-50 p-6 dark:bg-slate-900">
    ...
    <ul className="mt-4 space-y-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
      <li>Gmail OAuth approval</li>
      <li>Cloudflare API token authorization</li>
      <li>AmberOwl and SilverForge active usage</li>
      <li>Load testing under concurrent agents</li>
      <li>Disaster recovery drill (untested in practice)</li>
    </ul>
  </div>
</div>
...
1. **Get the server**: Hetzner, DigitalOcean, or similar. 4GB RAM minimum, 8GB comfortable.
...
The Gmail OAuth and Cloudflare token will arrive when they arrive. Until then, the system works without them. That is the point: build what you can verify, ship what you can maintain, and be honest about what remains pending.
