...
## The card and figure treatment

The solutions cards use two different image treatments, because the two kinds of images need different things. Diagram-like solution cards — the Excalidraw-sourced diagrams used by the agent, measurement, and operational capability entries — render **contained** with padding, so no information is cropped at the edges. Screenshot-based product cards — the Local LLM Lab guide, the Creative Observatory workbench, the hackathon voting app, and similar live deployments — may use cover cropping, because the edges of a screenshot are safe to cut and containment would leave dead space around the live UI.

There is no separate diagram inside this article, because the live cards on the solutions index already do that teaching job better than any additional figure would. The article's job is to explain the model in prose; the inventory's job is to show the model in the same place the evidence lives.
