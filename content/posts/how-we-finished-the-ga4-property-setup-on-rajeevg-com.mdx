...
date: 2026-03-24
updated: 2026-03-24
description: A granular walkthrough of the rajeevg.com GA4 property setup, the exact reporting choices that mattered, the live screenshots captured during the work, and the agent toolchain used to get it done.
...
<div className="not-prose my-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
  ...
  <div className="rounded-2xl border bg-card p-4 shadow-sm">
    ...
    <p className="mt-2 text-sm leading-6 text-muted-foreground">Receives browser-side events through the first-party `/metrics` collection path rather than a thin client-only setup.</p>
  </div>
  <div className="rounded-2xl border bg-card p-4 shadow-sm">
...
| First-party collection path | `https://rajeevg.com/metrics` |
...
<ArticleFigure
  ...
  alt="GA4 realtime overview showing active users and project_click as a key event"
  eyebrow="GA4 Realtime"
  title="Fresh live activity after agent-triggered production interactions"
  caption="This screenshot was taken after the agent opened the production projects page, clicked a live project GitHub link, opened the production analytics post, and clicked a code-copy button. The realtime view shows the property active and the `project_click` key event visible."
/>
...
- data retention was set to the longer standard-property window used in this setup pass
...
### 3. One admin screen was less trustworthy than the API

During the pass, the Custom definitions admin page rendered an empty-looking table at the moment I checked it, even though the property definitely had promoted definitions.

That is the kind of moment where it is tempting to keep reloading the same screen and lose time.

Instead, I cross-checked with the Admin/Data API and treated that as the authority. The property’s promoted dimensions and metrics were confirmed there, which made the UI oddity a secondary issue rather than a blocker.
