...
## The newest tool broke the rule

The most recent addition is the most interesting, because it does not fit the pattern above. Browser Use Pi does not issue one browser command per model step — it runs its own agent loop, writes JavaScript into a persistent V8 REPL, and drives Chrome through raw CDP, building its own helpers as it goes. That is the "heavier architecture" the speed table punishes.

And the speed table does punish it: **53.5 seconds median over ten reps against the leader's four**, with three of those ten ending in a false "done" that never left the default filter. On the toy job, the extra machinery is pure overhead.

Then it did something no other tool in the set could. On the corpus, it is **the only harness that passes the CHANEL product-page tag inspection** — the task where the site's anti-bot behaviour defeats the thin CLIs two times out of nine. It also passed the crawlability, SEO-metadata and Porsche tag audits.

So the honest summary is that the two rankings are not just different, they are *orthogonal*. The tool you want for a five-second deterministic job is the tool that adds the least between the model and the browser. The tool you want when the page fights back is one that can write its way around a problem instead of pressing the next button. I do not yet have evidence that code-batching wins on long authenticated workflows — that is the next thing to test — but I have a clear counter-example to "thin always wins".
