...
<Figure src="/images/editorial/multi-agent-journey-map.svg" alt="Timeline of five multi-agent phases with verdicts: native subagents kept the instinct, measurable workers kept measurement, the Codex and OpenCode split and lifecycle experiments were deleted, and role-aware routing is the current system" caption="Two years of multi-agent work on one map: five phases, two deliberate deletions, and the one lesson that survived — measurement. The current system is the simplest thing on the timeline." slug="multi-agent-journey-map" date="29 August 2026" />
