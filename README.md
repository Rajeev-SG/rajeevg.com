...
### Tooltips overlay (shadcn/ui)

- Files: `src/components/mermaid-tooltips.tsx`, integrated in `src/app/blog/[slug]/page.tsx`.
- What it does:
  - Parses inline SVG anchors generated by Mermaid for `click ... href` directives.
  - Computes bounding boxes and renders absolute overlay anchors with shadcn `Tooltip` for accessible labels.
  - Applies rounded corners to node rectangles by setting `rx/ry` after hydration; CSS fallback present in `globals.css`.
  - External links open in a new tab with `rel="noreferrer noopener"`.
- Positioning:
  - Overlays are portaled into the article container and absolutely positioned relative to it.
  - Ensure the article section has `id="article-content"` and `className="relative"` (already set in `[slug]/page.tsx`).
- Accessibility/UX:
  - Overlay anchors use focus-visible rings; tooltips read from `<title>`, `aria-label`, `data-title`, or the URL.
  - Cursor is a pointer on both overlays and native SVG anchors.
- Usage in Mermaid:
  ...
  graph TD
    ...
    click B href "https://github.com/Rajeev-SG/blog-2025" "View repo"
  ```

  - The tooltip text comes from the string at the end of each `click ...` directive. If omitted, the URL is used.
